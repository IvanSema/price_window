...
function getChecked(array) {
    const checked__type = document.querySelector('.product-equipment__input:checked ~ p').innerHTML;

    array.forEach(i => {
        if (i.type === +checked__type) {
            document.querySelectorAll('.product-equipment__text--price').forEach(j => {
                if (+j.innerHTML === i.price) {
                    j.classList.add('product-equipment__text--price-checked')
                    document.querySelector('.product-equipment__value').innerHTML = i.price;
                } else {
                    j.classList.remove('product-equipment__text--price-checked');
                }
            })
        }
    })
...
function result(){
    let res = {}
    res.price = +document.querySelector('.product-equipment__text--price-checked').innerHTML;
    res.type = +document.querySelector('.product-equipment__input:checked ~ p').innerHTML;
    console.log(res);
}
...
const ProductEquipment = props => {
    return (
        <div className='product-equipment'>
            ...
            <div className='product-equipment__header-wrapper'>
                <div className='product-equipment__item'><p>Тип</p><p>Ціна</p></div>
                {props.array.map((elem, i) => (
                    <label className='product-equipment__item' onClick={() => { getChecked(props.array) }}>
                        <div className='product-equipment__price'>
                            <input type='radio' name='radio-btn' className='product-equipment__input' value={elem.price} defaultChecked={i === 0 ? true : false}></input>
                            <div className='product-equipment__input-btn'></div>
                            <p className='product-equipment__text product-equipment__text--type'>{elem.type}</p></div>
                        <div className={i === 0 ? 'product-equipment__text  product-equipment__text--price product-equipment__text--price-checked' : 'product-equipment__text  product-equipment__text--price'}>{i === 0 ? temp = elem.price : elem.price}</div>
                    </label>
                ))
                }
            </div>

            <div className='product-equipment__footer-wrapper'>
            <p className='product-equipment__value'>{temp}</p>
                <button className='product-equipment__btn' onClick={result}>
